exports.handler = async function(event) {
  ...
  try {
    ...
    if (type === 'action') {
      // Student verbalized an action — evaluate it against checklist and respond as examiner
      ...
      for (const [section, items] of Object.entries(checklist)) {
        for (const item of items) {
          if (!item.done) unchecked.push(item.label);
        }
      }
...
Your job is to:
1. Understand what the student just did or said, even if spelled wrong or phrased oddly
2. Acknowledge it briefly like a real examiner would
